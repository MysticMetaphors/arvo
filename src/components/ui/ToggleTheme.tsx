...
export const ToggleTheme = ({
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 200 && isScrollAppear == true) setIsVisible(true);
    ...
    <>
      <button
        ...
        className={cn(
          ...
          isVisible ? "opacity-100" : isScrollAppear ? "opacity-0 pointer-events-none" : "",
          isDark ? "hover:text-amber-400" : "hover:text-blue-500",
          className
        )}
        {...props}
      >
        {isDark ? <Sun className="h-6 w-6 text-yellow-500" /> : <Moon className="h-6 w-6" />}
      </button>
