...
import { BrandingSettings, CampaignManager, PayrollSystem, PosSystem, ReportGenerator, SecurityHub } from "./FeatureModules";
...
export type ViewState = 'login' | 'dashboard' | 'inventory' | 'pos' | 'crm' | 'payroll' | 'kanban' | 'marketing' | 'security' | 'users' | 'reports' | 'branches' | 'branding' | 'vendors' | 'audits' | 'transfers';
...
export default function InventorySystem({ deviceMode }: { deviceMode: DeviceMode }) {
  ...
  const renderContent = () => {
    switch (view) {
      case 'dashboard': return <DashboardCharts deviceMode={deviceMode} onGeneratePO={() => setPoBuilderOpen(true)} />;
      ...
      case 'vendors': return <VendorsPage isMobile={isMobile} />;
      case 'audits': return <AuditsPage isMobile={isMobile} />;
      case 'transfers': return <StockTransfer isMobile={isMobile} />;
      default: return <DashboardCharts deviceMode={deviceMode} onGeneratePO={() => setPoBuilderOpen(true)} />;
    }
  };
  ...
  if (view === 'login') {
    return (
      ...
      <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 pattern-grid-lg text-slate-800 relative">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-200 z-10" data-demo-reveal="scale">
          ...
          <form onSubmit={handleLogin} className="space-y-4" data-demo-stagger="fast">
            <div>
              ...
              <input
                ...
                data-demo-target="login.username"
            ...
            <div>
              ...
              <input
                ...
                data-demo-target="login.password"
                className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
            ...
            <button
              type="submit"
              data-demo-target="login.signin"
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-lg transition-all shadow-md hover:shadow-lg"
            >
  ...
  return (
    <div className="flex h-full text-slate-800 bg-slate-50 relative">
      ...
      {!isMobile && (
        <aside className={`${isTablet ? 'w-20' : 'w-64'} bg-slate-900 text-slate-300 flex flex-col shrink-0 transition-all duration-300 border-r border-slate-800`}>
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            ...
            <NavItem id="transfers" icon="fa-truck" label="Transfers" />
            <NavItem id="crm" icon="fa-users" label="CRM" />
            <NavItem id="payroll" icon="fa-money-bill-wave" label="Payroll" />
            <NavItem id="vendors" icon="fa-handshake" label="Vendors" />
            ...
            <NavItem id="audits" icon="fa-file-contract" label="Audit History" />
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        {isMobile && mobileMenuOpen && (
          <div className="absolute inset-0 bg-slate-900 z-50 flex flex-col animate-in slide-in-from-left duration-200">
             ...
             <nav className="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                <NavItem id="payroll" icon="fa-money-bill-wave" label="Payroll" />
                <NavItem id="vendors" icon="fa-handshake" label="Vendors" />
                <NavItem id="audits" icon="fa-file-contract" label="Audits" />
                <NavItem id="marketing" icon="fa-bullhorn" label="Marketing" />
        ...
        {/* Global overlays */}
        <POBuilder open={poBuilderOpen} onClose={() => setPoBuilderOpen(false)} isMobile={isMobile} />
        <AIAssistant />
      </div>
    </div>
  );
