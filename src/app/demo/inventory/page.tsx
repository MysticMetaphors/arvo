...
export default function InventoryPage() {
  ...
  return (
    <main className="min-h-screen w-full bg-black-primary pt-28 pb-12 px-4 flex flex-col items-center">
      
      {/* Device Toggle Toolbar */}
      <div className="mb-6 flex items-center gap-4 bg-gray-900 p-2 rounded-full border border-gray-700 shadow-xl z-20">
        <button 
          onClick={() => setDeviceMode('desktop')}
          className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${deviceMode === 'desktop' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
        >
          <i className="fa-solid fa-desktop"></i> Desktop
        </button>
        <button 
          onClick={() => setDeviceMode('tablet')}
          className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${deviceMode === 'tablet' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
        >
          <i className="fa-solid fa-tablet-screen-button"></i> Tablet
        </button>
        <button 
          onClick={() => setDeviceMode('mobile')}
          className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 transition-all ${deviceMode === 'mobile' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
        >
